...
namespace Demo.Scripts.Runtime
{
    ...
    public class FPSController : FPSAnimController
    {
        ...
        public void startmove()
        {
            ...
            {
                // 이동할 좌표 설정
                Vector3 teleportPosition = new Vector3(2.8f, 0f, -9.2f);

                // 바라보는 방향 설정 (0, -90, 0)
                Vector3 teleportRotation = new Vector3(0f, -90f, 0f);

                // 캐릭터의 위치와 회전을 즉시 설정
                controller.enabled = false; // 임시로 컨트롤러 비활성화
        ...
        {
            float moveX = Input.GetAxisRaw("Horizontal") * mouseSensitivity * Time.deltaTime;

            float moveY = Input.GetAxisRaw("Vertical") * mouseSensitivity * Time.deltaTime;
